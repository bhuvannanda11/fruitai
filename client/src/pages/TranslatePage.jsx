...
const TranslatePage = () => {
    ...
    const sendMessage = async (e) => {
        ...
        const personMessage = { type: 'person', message }
        const botMessage = {
            type: 'bot',
            message: 'The feature is under progress. Please try again later.'
        }

        setMessageLogs(prevMessageLogs => {
            return [...prevMessageLogs, personMessage, botMessage]
        })
        ...
        <>
            <div className="absolute left-0 top-0 w-full px-5 py-2 border-b border-gray-200 rounded-lg shadow-lg flex items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-purple-400 italic">Bot (Beta)</p>
                </div>
            </div>
